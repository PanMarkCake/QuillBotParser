...
def main():
    ...
    try:
        ...
        text = "Chrome is running and some profile files are locked, causing the copy to fail. Trying without copying the profile (direct use), which may still fail if Chrome is open. Checking the bot code for options Chrome is running and locking profile files. The script falls back to the original profile, but Chrome can't start while it's open. Close Chrome first, then rerun. Alternatively, trying with copy_profile=False (may still fail if Chrome is open)"
        print(f"\n--- Paraphrasing ---\nInput: {text}")
        ...
        # 2. Humanizing Example (Advanced Mode)
        text_human = "AI is a branch of computer science that aims to create intelligent machines."
        
        # Since we have a user profile configured, use Advanced mode (Premium)
        if USER_DATA_DIR:
            print(f"\n--- Humanizing (Advanced Mode) ---\nInput: {text_human}")
            res_human = bot.humanize(text_human, mode="Advanced")
            print(f"Result: {res_human}")
        else:
            print(f"\n--- Humanizing (Basic Mode) ---\nInput: {text_human}")
            res_human = bot.humanize(text_human, mode="Basic")
            print(f"Result: {res_human}")
